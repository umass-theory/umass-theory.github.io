...
<summary>**Part 1:** We formalize fairness for the $k$-server problem via $(\alpha,\beta)$-fairness: each server's cost is at most an $\alpha/k$ fraction of the total plus $\beta$. We show fairness can be achieved without losing competitiveness.
Offline, we transform any optimal solution into a fair one with $\alpha=1+\varepsilon$ and $\beta=O(diam \cdot \log k/\epsilon)$ while incurring only an additive $O(diam \cdot k\log k/\epsilon)$ cost.
...
If $\epsilon\in[0,1]$ bounds the multiplicative prediction error, we obtain competitive ratio $\max\{0.221,\frac{1-\epsilon}{1+\epsilon}\}$ for ROSP (improving [Fujii Yoshida '23]) and $\max\{0.262,\frac{1-\epsilon}{1+\epsilon}\}$ for COSP, demonstrating the benefit of combining predictions with arrival-order control.</summary>
